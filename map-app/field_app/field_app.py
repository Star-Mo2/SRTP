...
# 作为 SRTP 站点下的蓝图片段。不设 url_prefix：/api/* 与 /uploads/* 与 SRTP 不冲突，
# 仅把入口页改为 /map，避免抢 SRTP 的 /。模板走独立 template_folder。
...
@bp.route("/map")
def index():
    return render_template("field_index.html")
...
@bp.route("/api/points/restore", methods=["POST"])
...
@bp.route("/api/points/<point_id>/position", methods=["POST"])
...
@bp.route("/api/points/<point_id>/position", methods=["DELETE"])
...
@bp.route("/api/points/<point_id>/photos", methods=["POST"])
...
@bp.route("/api/points/<point_id>/photos/<int:photo_id>", methods=["DELETE"])
